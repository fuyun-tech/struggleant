...
import { IconDeepThinkingComponent } from 'src/app/icons/icon-deep-thinking.component';
...
export class AiChatComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit() {
    combineLatest([
      ...
      .pipe(
        skipWhile(([appInfo, options, user]) => isEmpty(appInfo) || isEmpty(options) || !user.id),
  ...
  private getConversation() {
    ...
    this.botConversationService
      ...
      .subscribe((res) => {
        ...
        if (res && res.id) {
          if (res.user) {
            res.user.avatarUrl = this.userService.getUserAvatar(
              res.user,
          ...
          this.avatarUrl.set(this.botService.getBotAvatar(appInfo, res.bot));
          ...
          this.messageLoading.set(false);
          this.avatarUrl.set(this.botService.getBotAvatar(appInfo));
          this.initGreeting();
        }
      });
  }
